...
function aplicarFiltros() {
    ...
    let ordenPrecio = document.getElementById('orden-precio').value;
    let ordenTitulo = document.getElementById('orden-titulo').value;

    let librosFiltrados = libros.filter(libro => {
        return (
            (busqueda === '' || libro.titulo.toLowerCase().includes(busqueda)) &&
            (categoria === '' || libro.categoria === categoria)
        );
    });

    if (ordenPrecio !== '' && ordenPrecio !== 'none') {
        if (ordenPrecio === 'asc') {
...
function mostrarLibros(libros) {
    ...
    libros.forEach(libro => {
        let itemLibro = document.createElement('div');
        itemLibro.classList.add('grid-item');

        let enlaceDetalle = document.createElement('a');
        enlaceDetalle.href = `detalle-libro.html?id=${libro.id}`;
        itemLibro.appendChild(enlaceDetalle);

        let nombreLibro = document.createElement('p');
        nombreLibro.textContent = libro.titulo;
        itemLibro.appendChild(nombreLibro);

        let imagenLibro = document.createElement('img');
        imagenLibro.src = libro.imagen;
        enlaceDetalle.appendChild(imagenLibro);

        let precioLibro = document.createElement('p');
        precioLibro.textContent = `Precio: $${libro.precio}`;
        itemLibro.appendChild(precioLibro);
...
function limpiarFiltros() {
    ...
    document.getElementById('orden-precio').value = 'none';
    document.getElementById('orden-titulo').value = 'none';
